...
abstract class BaseCompany extends BaseObject  implements Persistent {
  ...
  const PEER = 'CompanyPeer';
	...
	/**
	 * The value for the image_large field.
	 * @var        string
	 */
	protected $image_large;

	/**
	 * @var        array Policy[] Collection to store aggregation of Policy objects.
	 */
	protected $collPolicys;

	/**
	 * @var        Criteria The criteria used to select the current contents of collPolicys.
	 */
	private $lastPolicyCriteria = null;
	...
	/**
	 * Get the [image_large] column value.
	 * 
	 * @return     string
	 */
	...
	public function setCid($v)
	{
		...
		if ($this->cid !== $v) {
			$this->cid = $v;
			$this->modifiedColumns[] = CompanyPeer::CID;
		}
	...
	public function setImageSmall($v)
	{
		...
		if ($this->image_small !== $v) {
			$this->image_small = $v;
			$this->modifiedColumns[] = CompanyPeer::IMAGE_SMALL;
		}
	...
	/**
	 * Set the value of [image_large] column.
	 * 
	 * @param      string $v new value
	 * @return     Company The current object (for fluent API support)
	 */
	public function setImageLarge($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->image_large !== $v) {
			$this->image_large = $v;
			$this->modifiedColumns[] = CompanyPeer::IMAGE_LARGE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->image_small = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->image_large = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setCid($pk);  //[IMV] update autoincrement primary key
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			$keys[0] => $this->getCid(),
			$keys[1] => $this->getName(),
			$keys[2] => $this->getImageSmall(),
			$keys[3] => $this->getImageLarge(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setImageSmall($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setImageLarge($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CompanyPeer::CID)) $criteria->add(CompanyPeer::CID, $this->cid);
		if ($this->isColumnModified(CompanyPeer::NAME)) $criteria->add(CompanyPeer::NAME, $this->name);
		if ($this->isColumnModified(CompanyPeer::IMAGE_SMALL)) $criteria->add(CompanyPeer::IMAGE_SMALL, $this->image_small);
		if ($this->isColumnModified(CompanyPeer::IMAGE_LARGE)) $criteria->add(CompanyPeer::IMAGE_LARGE, $this->image_large);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CompanyPeer::CID, $this->cid);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setImageLarge($this->image_large);
	...
	/**
	 * Clears out the collPolicys collection (array).
	 *
	...
	/**
	 * Initializes the collPolicys collection (array).
	 *
	 ...
	 * @return     void
	 */
	public function initPolicys()
	{
		$this->collPolicys = array();
	}

	/**
	 * Gets an array of Policy objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Company has previously been saved, it will retrieve
	 * related Policys from storage. If this Company is new, it will return
		...
		if ($this->collPolicys === null) {
			if ($this->isNew()) {
			   $this->collPolicys = array();
				...
				$criteria->add(PolicyPeer::CID, $this->cid);

				PolicyPeer::addSelectColumns($criteria);
				$this->collPolicys = PolicyPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				PolicyPeer::addSelectColumns($criteria);
				if (!isset($this->lastPolicyCriteria) || !$this->lastPolicyCriteria->equals($criteria)) {
					$this->collPolicys = PolicyPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastPolicyCriteria = $criteria;
		return $this->collPolicys;
	}

	/**
	 * Returns the number of related Policy objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Policy objects.
	 * @throws     PropelException
	 */
	public function countPolicys(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collPolicys === null) {
			if ($this->isNew()) {
				...
				$count = PolicyPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	...
	/**
	 * Method called to associate a Policy object to this object
	 * through the Policy foreign key attribute.
	 *
	 * @param      Policy $l Policy
	 * @return     void
	 * @throws     PropelException
	 */
	public function addPolicy(Policy $l)
	{
		if ($this->collPolicys === null) {
			$this->initPolicys();
		}
		if (!in_array($l, $this->collPolicys, true)) { // only add it if the **same** object is not already associated
			array_push($this->collPolicys, $l);
			$l->setCompany($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collPolicys) {
				foreach ((array) $this->collPolicys as $o) {
