...
sub process($$){
	my ($url, $method, $scrape_data, $spoof, $db_text, $live_text, $flags);
	my ($pid, $verbose) = @_;
	...
	$latest_sth->bind_param(1, $pid, DBI::SQL_INTEGER);
	$latest_sth->execute or die "Could not execute latest version retrieval for $pid\n";
...
# Go through results
my $workerpool = MultiThread::WorkerPool->new( EntryPoint => \&process);
my @tickets;
while(my $policy_rowref = $sth->fetchrow_arrayref()){
#	push @tickets, $workerpool->send_request($$policy_rowref[0], $verbose);
	process($$policy_rowref[0], $verbose);
}
